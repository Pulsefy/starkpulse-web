...
export const useGlobalErrors = () => {
  ...
  const errors = [authError, portfolioError].filter(Boolean);
  return errors;
};
...
export const useClearAllErrors = () => {
  ...
  const clearPortfolioError = usePortfolioStore((state: any) => state.clearError);
...
if (typeof window !== 'undefined' && process.env.NODE_ENV === 'development') {
  ...
  console.log('🛠️ StarkPulse dev tools available at window.__STARKPULSE_DEV_TOOLS__');
} 